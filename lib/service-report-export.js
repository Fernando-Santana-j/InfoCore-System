...
function writeServiceReportPdf(service, uploadDir, meta = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        stages.forEach((item, index) => {
            const typeLabel = ({ service: 'Serviço', part: 'Peça/material', diagnostic: 'Diagnóstico', other: 'Outro' })[item.workType] || 'Serviço';
            doc.fontSize(12).fillColor('#0f172a').text(`Etapa ${index + 1} · ${typeLabel}: ${item.label || 'Serviço'}`);
            doc.fontSize(10).fillColor('#475569');
            if (item.customerNote) doc.text(`Relato: ${item.customerNote}`);
            if (item.techNote) doc.text(`Serviço: ${item.techNote}`);
            doc.text(`Status: ${item.archived ? 'Arquivada' : (item.done ? 'Concluído' : 'Em andamento')}`);
            doc.moveDown(0.6);
        });
